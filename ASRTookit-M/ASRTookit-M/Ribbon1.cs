...
namespace ASRTookit_M
{
    public partial class Ribbon1
    {
            ...
            foreach (Excel.Range row in selection.Rows)
            {
                foreach (Excel.Range cell in row.Cells)
                {
                    ...
                    if (cell.Value is double)
                    {
                        ...
                        if (cell.HasFormula)
                        {
                            String formula = cell.Formula.Substring(1);
                            if (formula.StartsWith("-(") && formula.EndsWith(")"))
                            {
                                cell.Formula = "=" + formula.Substring(1);
                                emptyCellCount = 0;
                                continue;
                            }
                            if (formula.StartsWith("(") && formula.EndsWith(")"))
        ...
        {
            SheetSelectionForm selectionForm = new SheetSelectionForm();
            if (selectionForm.ShowDialog() == DialogResult.OK)
            {
                List<string> selectedSheets = selectionForm.GetSelectedSheets();
                if (selectedSheets.Count == 0)
                {
                    MessageBox.Show("请选择至少一个工作表。");
                    return;
                }
                // 调用数据合并处理逻辑
                bool success = MergeHelper.MergeSheets(selectedSheets);
                if (success)
